...
function SelfProject() {
  ...
  const selfProjectRef = useRef();
  useEffect(() => {
    if (theme === "light") {
      selfProjectRef.current.style.boxShadow = "0 0 3px 1px #ccc";
      selfProjectRef.current.style.backgroundColor = "#fff";
    }
    if (theme === "dark") {
      selfProjectRef.current.style.boxShadow = "";
      selfProjectRef.current.style.backgroundColor = "#323233";
    }
  }, [theme]);
  const { t } = useTranslation();
  return (
    <div className="self-project" ref={selfProjectRef}>
